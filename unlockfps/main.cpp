...
DWORD __stdcall Thread2(LPVOID p)
{
    if (!p)
        return 0;
    ...
    while (!bStop)
    {
      try {
        ...
        printf("\rWrong input format, enter an integer");
      }
        ...
        if (1<i && i<200)
        {
            fps = i;
              ...
              printf("\rFps value must be between 1 and 200");
        }
        printf("\rFPS: %d - %s    ", fps, fps > 60 ? "ON" : "OFF");
        *pTargetFPS = fps;
    }
...
int main(int argc, char** argv)
{
    ...
    */
    ...
    printf("Done\n\n");
    printf("Use arrow keys to change FPS limit:\n");
    printf("  HOME+UP:    +20\n");
    printf("  HOME+DOWN:  -20\n");
    printf("  HOME+LEFT:  -2\n");
    printf("  HOME+RIGHT: +2\n");
    ...
    // keybinds thread   
    HANDLE hThread2 = CreateThread(nullptr, 0, Thread2, &TargetFPS, 0, nullptr);
    HANDLE hThread1 = CreateThread(nullptr, 0, Thread1, &TargetFPS, 0, nullptr);
